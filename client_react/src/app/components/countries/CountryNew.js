...
const CountryNew = () => {
    ...
    // const { data: country = [], isLoadingC, isSuccessC, isErrorC, errorC, refetchC } = useG
    //--------------------------------------//
    const [products, setProducts] = useState([]);
    const [layout, setLayout] = useState('grid');
    // const country =str.split("=")[1];
    ...
    const gridItem = (product) => {
        return (
            <div className="col-12 sm:col-6 lg:col-12 xl:col-4 p-2" key={product._id}>
                {/* <img className="picturesizebig"src={`http://localhost:7788/uploads/${.pictures[0]}`} alt={product.name} /> */}
                {/* <img  src={`http://localhost:7788/uploads/${product.picture.split("\\")[2]}`} alt={product.name} /> */}
